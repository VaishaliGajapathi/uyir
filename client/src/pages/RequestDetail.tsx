        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={!(r.status === "verified" || r.status === "alert_sent" || r.status === "donor_accepted" || r.status === "completed")}
            >
...
Request link: ${shareLink}`;
          ...
          {!(r.status === "verified" || r.status === "alert_sent" || r.status === "donor_accepted" || r.status === "completed") && (
              ...
              return (
                <div key={x.id} className="rounded-xl bg-slate-50 px-3 py-2">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-slate-700">{x.donor?.name || "Donor"} · {x.donor?.bloodGroup}</p>
                      <p className="text-[11px] text-slate-400">{x.distanceKm != null ? `${x.distanceKm} km` : x.donor?.district} {x.etaMinutes ? `· ~${x.etaMinutes} min` : ""} · score {x.matchScore}</p>
                    </div>
        ...
        {isOwner && !["completed", "closed"].includes(r.status) && (
          <div className="grid grid-cols-2 gap-3">
